...
(function(){
  ...
  async function loadQuestions(){
    ...
    try {
      ...
      (statuses || []).forEach(r => {
        if (r.is_published === false || r.is_deleted === true) byId.delete(r.id);
      });
      window.KagakuData.questions = Array.from(byId.values());
      console.log('KagakuData.questions 更新完了:', window.KagakuData.questions.length, '問');
      console.log('クラウド問題:', (rows || []).map(q => q.question));
    } catch(e) {
  ...
  async function syncSnapshot(){
    ...
    try {
      ...
      const mastery = values.length
        ? Math.round(values.reduce((a,b) => a + Mastery.questionScore(b), 0) / values.length)
        : 0;
  ...
  async function restoreSnapshot(){
    const c = getClient();
    if (!c || !currentUser || !window.Storage) {
      restored = true;
      return;
    }
    try {
      const { data: row, error } = await c
        ...
        .maybeSingle();
      if (!error && row?.data) {
        Storage.saveUserData(row.data, { skipSync: true });
      } else if (error) {
        console.warn('クラウド進捗の取得に失敗:', error.message);
      }
    } catch(e) {
  ...
  async function refreshAuth(){
    ...
    if (error) console.warn('Supabase認証状態の取得に失敗:', error.message);
  ...
  async function handleAuthSession(session){
    ...
    document.dispatchEvent(new CustomEvent('kagaku-auth-change', {
      detail: { user: currentUser }
    }));
  }

  async function initializeAuth(){
    if (authInitialized) return authReady;
    ...
    // 認証イベントは初期セッションだけ無視し、初期状態はgetSession()で確定します。
  ...
  async function signUp(email,password,displayName){
    ...
    const result = await c.auth.signUp({
      email,
      password,
      options: { data: { display_name: displayName || '学習者' } }
    });
    if (!result.error && result.data?.session) {
      await handleAuthSession(result.data.session);
    }
  ...
  // 問題取得と認証復元の両方が終わってから、演習ページが開始できるようにします。
  const questionsPromise = loadQuestions();
  window.KagakuCloud.questionsReady = Promise.all([
    questionsPromise,
    authReady
  ]).then(() => undefined);

  // scriptがbody末尾にあっても、他ページの読み込み方式に依存しないよう即時初期化します。
  void initializeAuth();
})();
