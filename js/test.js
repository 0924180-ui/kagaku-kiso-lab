              ...
              (q.options || [])
                .map((option, index) => `
                  <button
                    ...
                    style="${selected === index ? 'border-color: var(--accent, #2f6f4e); background: var(--green-pale, #edf6ef); box-shadow: 0 0 0 2px rgba(47,111,78,.16);' : ''}"
                    data-opt="${index}"
                    ${
                      selected !== undefined
                        ? "disabled"
                  ...
                  >
                    ...
                    ${esc(option)}
                    ${selected === index ? '<span style="margin-left:10px; font-weight:700;">✓ 選択中</span>' : ''}
                  </button>
                `)
                .join("")
            }
