...
/*
 * 重要:
 * 管理画面では、学習者側がSupabaseへ保存した
 * mastery / attempt_count / total_time_seconds /
 * unit_mastery を優先して使用する。
 *
 * これにより、管理画面側で問題一覧が増減した場合でも
 * 学習者側と違う問題数・定着度を再計算してしまわない。
 */
      ...
      (day, i) => {
        ...
        for (
          ...
          total += Math.max(
            0,
            Number(
              cur.total_time_seconds ||
              0
            ) -
              (
                old
                  ? Number(
                      old.total_time_seconds ||
                      0
                    )
                  : 0
              )
...
function exportQuestionsCSV() {
  ...
  downloadCSV(
    `kagaku_lab_questions_${new Date()
      .toISOString()
      .slice(0, 10)}.csv`,
    rows
      .map(
